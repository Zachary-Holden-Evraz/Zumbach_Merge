...
# Make a stop button so that we can stop at any time without corrupting the excel file
# We will put the stop button after certain blocks of code instead of a large while loop because we don't want it to be writing any data when the program stops
running = 1
thread_pool_executor = futures.ThreadPoolExecutor(max_workers = 1)
def stop():
    running = 0
stopButton = tk.Button(root_win, text = "Stop", command = stop)
...
# We need to set up Red_data early since it is in a different (parent) directory
try:
    os.chdir(dir_with_files)
    for file in os.listdir(dir_with_files):
